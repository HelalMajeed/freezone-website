/** SPA: catalog/site data comes from Express unless static-only mode is enabled (dev only). */
...
export function getDatabaseBlockReason(): DatabaseBlockReason {
  if (!import.meta.env.PROD && import.meta.env.VITE_STATIC_STOREFRONT_ONLY === "true") return "static_only";
  return "none";
}
