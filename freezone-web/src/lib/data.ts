/** Attribute types (Universal Product Classification Framework). */
export type AttributeType = "SELECT" | "MULTI_SELECT" | "RANGE" | "BOOLEAN" | "TEXT" | "COLOR";

/** Bilingual attribute — `CategoryAttribute` table or legacy `Category.facetKeys` JSON. */
...
export interface Product {
  id: number;
  cat: string;
  /** Additional category slugs (browse/filter); spec facets use `cat` (primary) only. */
  extraCats?: string[];
  brand: string;
  name: string;
  desc: string;
