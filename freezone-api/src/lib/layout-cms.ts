...
export type HomeCmsPayload = {
  ...
  /** CSS `background` for the trust strip */
  ...
  hero: {
    ...
    scrimOpacity: number;
    /** Resolved CSS `color` for prev/next chevrons */
...
function staticHomeCms(locale: LocaleCode): HomeCmsPayload {
  const en = locale === "en";
  return {
...
export async function getHomeCms(locale: LocaleCode): Promise<HomeCmsPayload> {
  ...
  try {
    const [cfg, trust, spots, heroes, showroomRows, promoRows] = await Promise.all([
      prisma.siteConfig.findUnique({ where: { id: 1 } }),
      prisma.trustBarItem.findMany({ orderBy: { sortOrder: "asc" } }),
      prisma.homeSpotlightItem.findMany({
        where: { active: true },
    ...
    const en = locale === "en";
    const base = composeHomeCmsFromParts(
      cfg as SiteCfgExt,
      trust as { id: number; textEn: string; textAr: string; iconKey: string }[],
      spots as {
        id: number;
        labelEn: string;
        labelAr: string;
      ...
      heroes as CmsHeroSlideRow[],
      locale,
    );

    const showroom: PublicShowroomItem[] = showroomRows.map((s) => ({
      id: s.id,
      url: s.url,
      kind: s.kind,
      title: en ? (s.titleEn ?? "").trim() || (s.titleAr ?? "").trim() : (s.titleAr ?? "").trim() || (s.titleEn ?? "").trim(),
    }));

    const promoBanners: PublicPromoBanner[] = promoRows.map((p) => ({
      id: p.id,
    ...
    return { ...base, showroom, promoBanners };
  } catch (e) {
    if (!isDbConnectionError(e)) console.error("[layout-cms] DB error, using static:", e);
    return staticHomeCms(locale);
  }
}
